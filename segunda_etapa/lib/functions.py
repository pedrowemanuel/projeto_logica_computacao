...
def cnf_para_dimacs(formula_cnf, nome_arquivo, regras):
    ...
    for elemento in lista:
        atomicas[str(elemento)] = index
        index += 1
    
    # convertendo a formula
    lista_cnf = conjuncao_clausulas_para_lista(formula_cnf)

    lista_cnf = converte_atomica_para_numero(lista_cnf, atomicas)
...
def percorrer_clausulas(clausula, dicionario):
    lista_clausula = []
    for item in clausula:
        recursive_clausulas(item,dicionario, lista_clausula)
        
    return lista_clausula

def recursive_clausulas(formula, dicionario, lista_clausula):
    
    if isinstance(formula, Or):
        recursive_clausulas(formula.left,dicionario, lista_clausula)
        recursive_clausulas(formula.right,dicionario, lista_clausula)
        
    elif isinstance(formula, Not):
        lista_clausula.append(dicionario[str(formula.inner)]*(-1))
    elif isinstance(formula, Atom):
        lista_clausula.append(dicionario[str(formula)])


def converte_atomica_para_numero(formula, dicionario):
    lista_index = []
    for clausula in formula:
        lista_index.append(percorrer_clausulas(clausula, dicionario))
    return lista_index
    #print(lista_index)
